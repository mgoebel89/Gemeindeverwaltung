  ...
  router.post('/', async (req, res) => {
    ...
    try {
      ...
      if (post.status !== 201 || !post.headers.location) {
        return res.status(502).json({ error: `Scan-Job abgelehnt (Status ${post.status}). Papier eingelegt? Quelle korrekt (Einzug/Vorlagenglas)?` });
      }
      let jobUrl = post.headers.location;
      if (/^\//.test(jobUrl)) { const u = new URL(base); jobUrl = `${u.protocol}//${u.host}${jobUrl}`; }

      // 2) Seiten abholen (NextDocument bis nichts mehr kommt)
      const created = [];
      for (let page = 0; page < 50; page++) {
        ...
        try {
          ...
          if (created.length) break; // Netzwerk-Ende nach mind. einer Seite tolerieren
          throw e;
        }
        ...
        fs.writeFileSync(db.belegFilePath(auslageId, id), doc.body);
        const rec = db.insertBelegFile({
          id, auslageId,
          filename: `Scan-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}-${page + 1}.jpg`,
          mimetype: 'image/jpeg',
      ...
      if (!created.length) return res.status(502).json({ error: 'Scanner lieferte keine Seite. Papier eingelegt?' });
